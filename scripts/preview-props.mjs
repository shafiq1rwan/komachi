...
const server=await createServer({server:{open:false,port:4191,strictPort:true}});await server.listen();let browser;
try{
 ...
 await page.goto('http://localhost:4191/docs/props-preview.html',{waitUntil:'networkidle0'});await page.waitForFunction(()=>window.propsPreview);mkdirSync('docs/props',{recursive:true});
 await page.screenshot({path:'docs/props/komachi-props-characters.png'});
 const result=await page.evaluate(async()=>{
  ...
  const c=p.chars[0],old=c.accessory;clearCharacterProp(c);if(old.parent||c.accessory)throw new Error('Detach failed');equipCharacterProp(c,'shopping-bag');
  return{assets:results,lowestWalkingPoint:lowest};
 });
 await page.select('#view','props');await page.screenshot({path:'docs/props/komachi-props-preview.png'});
 await page.click('#umbrella');await page.screenshot({path:'docs/props/komachi-props-closed.png'});
